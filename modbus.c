...
void modbus_serial_putc_modem(int8 c){
   fputc(c, MODEM);
   modbus_calc_crc(c);
   delay_us(1000000/MODBUS_SERIAL_BAUD); //one stop bit.  not exact
}
                           ...
                           int16 *values)
{
   ...
   modbus_serial_putc_modem(make8(start_address,1));
   modbus_serial_putc_modem(make8(start_address,0));

   modbus_serial_putc_modem(make8(quantity,1));
   modbus_serial_putc_modem(make8(quantity,0));
   
   modbus_serial_putc_modem(count);

   for(i=0; i < quantity; ++i)
   {
      modbus_serial_putc_modem(make8(values[i],1));
      modbus_serial_putc_modem(make8(values[i],0));
   }
