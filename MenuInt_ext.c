...
char IDslave[2];// Direccion esclavo
char timerpt[5];
int esclavo;
int panel;
int contrpt;
...
int i;
	...
	fprintf(COMV," G-Ingrese ID de esclavo a leer\n\r");
	...
	do{
		...
		switch(c){
				...
				case 'G': case 'g':
						contrpt=0;
						for(i=0;i<2;i++) IDslave[i]=0x00;
						fprintf(COMV,"\n\rIngrese ID de esclavo[entre (0~99)]: ");
						do{
							c=getc(COMV);
							putc(c,COMV);
								if(c!=0x08&&c!=0x0D)
									...
									if(contrpt>0&&c!=0x0D)
										IDslave[--contrpt]=0x00;
								} 	
						}while(c!=0x0D);	

						esclavo=atoi(IDslave);
						write_eeprom(500,esclavo);
						delay_ms(5);
						esclavo=read_eeprom(500);	
						fprintf(COMV,"\r\n%i\r\n",esclavo);
